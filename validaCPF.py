...
def limpaCPF(CPFaSerVerificado):
    CPFlimpo = CPFaSerVerificado.replace('.', '')
    CPFlimpo2 = CPFlimpo.replace('-', '')
    return CPFlimpo2

def compararOsCPFs():
    cpf = limpaCPF(CPFaSerVerificado)
    verificador = dv(cpf[:9])
    if cpf == cpf[0:9] + verificador:
        return 'CPF Valido!!!'
...
def gera_lista_cpfs(cpfSolicitados):
    listaDeCPFs = []
    while cpfSolicitados > 0:
        cpf = gera_cpf_valido()
        digitoDeVerificacao = dv(cpf)
        cpf = cpf[:3] + '.' + cpf[3:6] + '.' + cpf[6:9]
        cpfFinal = cpf + '-' + digitoDeVerificacao
        listaDeCPFs.append(cpfFinal)
        cpfSolicitados -= 1 
    return listaDeCPFs

if __name__ == '__main__':
    escolhaUmaOpcao = input("\nSe deseja validar um CPF digite 1, Se deseja gerar CPF digite 2:")

    if escolhaUmaOpcao == '2':
        cpfSolicitados = int(input('\nQuantos CPFs deseja:'))
        listaDeCPFs = gera_lista_cpfs(cpfSolicitados)
        print(listaDeCPFs)
    elif escolhaUmaOpcao == '1':
        CPFaSerVerificado = input('\nDigite o CPF:' )    
        CPFverificado = compararOsCPFs()
        print(CPFverificado)    